...
-- holds the data about the current match (also is the map the server starts on)
currentMatch = testDMMatchSettings.fort_dm--VArena_dm--fort_dm
...
function Reset(pid) --resets cells 
	...
	if p ~= nil and p:IsLoggedIn() then
		...
		logicHandler.ResetCell(pid, "Vivec, Arena Pit")
    end
end
...
function EndIt() -- Ends the round and starts a new one? Maybe?
    ...
    for pid, p in pairs(Players) do
        ...
        elseif lifenumber == bestlifenumber and numberofbestlifepeople == 1 and Players[pid].data.mwTDM.inarena == 1 then
		      --  logicHandler.RunConsoleCommandOnPlayer(pid, 'player->AddItem "Gold_001" 50') --adds 50 gold for winning
            for pid, p in pairs(Players) do
               -- tes3mp.SendMessage(pid, color.Yellow .. nameon .. " has won with " .. bestlifenumber .. " lives! 50 gold prize to them!\n")
                tes3mp.SendMessage(pid, color.Yellow .. nameon .. " has won with " .. bestlifenumber .. " lives!\n")
            end
        end
	end
    end
	   ...
	   --[[ for pid, p in pairs(Players) do  --adds 10 gold per life left
			if p ~= nil and p:IsLoggedIn() and Players[pid].data.mwTDM ~= nil then
		...
		for pid, p in pairs(Players) do
		    ...
		    if Players[pid].data.mwTDM.status == 1 and Players[pid].data.mwTDM.inarena == 1 then
			    --logicHandler.RunConsoleCommandOnPlayer(pid, 'player->AddItem "Gold_001" 50')  --adds 50 gold for surviving
			    local nameon = Players[pid].data.login.name
...
testDM.EndCharGen = function(pid) -- happens at End of "charactergeneration"?
	...
	logicHandler.RunConsoleCommandOnPlayer(pid, 'player->AddItem "Gold_001" 300')
	testDM.PlayerInit(pid)
end
...
testDM.alivecount = function(pid)
--local alivecount = 0
...
customCommandHooks.registerCommand("alive", testDM.alivecount)
